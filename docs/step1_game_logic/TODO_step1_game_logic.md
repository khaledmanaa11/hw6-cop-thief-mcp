...
- **Starting positions (deterministic):** Cop at `(0, 0)`; Thief at
  `(rows-1, cols-1)`. They are always distinct for any grid ≥ 2×2.
...
- [x] **A1** — `requirements.txt` — create — list runtime + test deps, one per line:
      `pyyaml` and `pytest`. Then install: `pip install -r requirements.txt`.
      **Check:** `python -c "import yaml, pytest"` exits 0.
- [x] **A2** — `pyproject.toml` — create — add a minimal pytest config so imports resolve
      from the repo root:
      ```toml
...
- [x] **A3** — `config.yaml` — create — add root-level game parameters:
      ```yaml
      grid_size: [5, 5]
      max_moves: 25
      num_games: 6
      ...
      scoring:
        ...
        thief_loss: 5
      ```
      **Check:** file is valid YAML — `python -c "import yaml; yaml.safe_load(open('config.yaml'))"` exits 0.
- [x] **A4** — `src/game/__init__.py` — create — empty file to make `src.game` an importable package.
      **Check:** `python -c "import src.game"` exits 0 (run from repo root).

## Phase B — config & models
- [x] **B1** — `src/game/config.py` — create — define `ScoringConfig` and `Config` dataclasses
...
- [x] **B2** — `src/game/board.py` — create — implement `class Board` constructed as
...
- [x] **B3** — `src/game/state.py` — create — define `GameState` dataclass with fields:
...
- [x] **B4** — `src/game/state.py` — extend — add `def initial_state(config) -> GameState:`
...
## Phase C — moves & rules
- [x] **C1** — `src/game/moves.py` — create — define `class Move(Enum)` with the 8 directional
...
- [x] **C2** — `src/game/moves.py` — extend — implement
...
- [x] **C3** — `src/game/moves.py` — extend — implement
      ...
      `moves_used` by 1 and flip `to_move` (`THIEF`↔`COP`). Raise `ValueError` if `move` is not in
      `legal_moves(state)`.
...
- [x] **C4** — `src/game/rules.py` — create — implement
...
## Phase D — movers
- [x] **D1** — `src/game/movers.py` — create — declare `class Mover(Protocol):` with
...
## Phase E — results & engine
- [x] **E1** — `src/game/engine.py` — create — define two dataclasses at the top of the file:
...
- [x] **E2** — `src/game/engine.py` — extend — implement
      `def play_sub_game(config, cop_mover, thief_mover) -> SubGameResult:`.
      Build `initial_state(config)`. Loop:
      (a) if `is_capture(state)` → winner `"COP"`, break;
      (b) if `is_timeout(state, config)` → winner `"THIEF"`, break;
...
- [x] **E3** — `src/game/engine.py` — extend — implement
      `def play_series(config, group_a, group_b) -> SeriesResult:` that runs `config.num_games`
      sub-games between two **groups**, **swapping Cop/Thief roles each sub-game** (even index →
      A=Cop/B=Thief, odd → A=Thief/B=Cop). Tag each result's `cop_group`/`thief_group`, and add each
      role's points to the group that played it → `group_a_total`, `group_b_total`. Per §4.4 each
      group plays Cop 3× and Thief 3×, so its total stays within [30, 90].
...
- [x] **E4** — `src/game/__main__.py` — create — a CLI entry: load `config.yaml`, run
...
## Phase F — tests
- [x] **F1** — `tests/test_game/__init__.py` — create — empty file so the test package imports.
      **Check:** file exists.
- [x] **F2** — `tests/test_game/test_config.py` — create — assert `load_config('config.yaml')`
...
- [x] **F3** — `tests/test_game/test_board.py` — create — assert `in_bounds`/`is_blocked` behave,
...
- [x] **F4** — `tests/test_game/test_rules.py` — create — assert `is_capture` true when positions
...
- [x] **F5** — `tests/test_game/test_moves.py` — create — assert movement .is 8-directional, that an
...
- [x] **F6** — `tests/test_game/test_engine.py` — create — parameterize over grid sizes
...
- [x] **F7** — full suite — run everything and confirm no hard-coded game constants leaked.
...
## Phase G — verify
- [x] **G1** — `config.yaml` — verify — confirm it still contains the exact required keys/defaults
...
- [x] All PRD acceptance criteria hold (`PRD_step1_game_logic.md` §7).
...
- [x] `docs/_system/ROADMAP.md` is updated to mark step 1 as ✅ (Developer does this at the end).
